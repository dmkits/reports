...
FROM (
    select p.PCatID, DocName='Продажа товара оператором', SUM(d.Qty) AS TotalQty, SUM(d.RealSum) AS TotalSum
    from t_sale s
        ...
        GROUP BY p.PCatID
    UNION
    select p.PCatID, DocName='Возврат товара по чеку', SUM(d.Qty) AS TotalQty, -SUM(d.RealSum) AS TotalSum
    from t_CRRet r
