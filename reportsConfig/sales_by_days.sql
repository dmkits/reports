...
from
(
   select  d.ProdID,s.DocDate , DocName='Продажа товара оператором',/* pg2.PGrName2, pg3.PGrName3, p.Article1, p.ProdName,p.UM,*/  SUM(d.Qty) AS TotalQty, d.RealPrice as PriceCC_wt, SUM(d.RealSum) AS TotalSum
       from t_sale s
   ...
   UNION
   select d.ProdID,r.DocDate,DocName='Возврат товара по чеку',/* pg2.PGrName2, pg3.PGrName3, p.Article1, p.ProdName,p.UM,*/  SUM(d.Qty) AS TotalQty, d.RealPrice as PriceCC_wt, -SUM(d.RealSum) AS TotalSum
   from t_CRRet r
